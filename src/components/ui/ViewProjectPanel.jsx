...
export default function ViewProjectPanel({
    ...
    const toggleFavorite = () => {
        const token = cookies.token
        if (favorite) {
            // What happens here is that it assumes the value of favorite count and only changes if it encounters an error
            destroyFavorite(id, token).catch(() => {
                ...
                <ScrollArea className='bg-white h-[90vh] px-10 w-full'>
                    {loading ?
                        ...
                        <div className='flex flex-col gap-10 items-center pb-10'>
                            ...
                            {(file_type === 'pdf' || (file_type === '' && response?.file?.trim().length !== 0)) &&
                                <>
